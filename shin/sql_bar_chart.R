sql_bar_chart <- function(query){
  df_win_count <- dbGetQuery(conn,query)
  #barplot(df_win_count[,2],names.arg=df_win_count[,1])
  return (list((wincount=df_win_count[,2]),(names.arg=df_win_count[,1])))
}

res <- list(sql_bar_chart("SELECT 
                           A.WINNER TEAM,
